 ...
 */
export function dataAvailabilityTransform(tree: GenericParent, file?: VFile) {
  ...
  dataAvailability.forEach((node) => {
    ...
    titles.forEach((title) => {
      if (toText(title).toLowerCase() === 'data availability') {
        jatsFileWarn(file, 'Removed redundant Data Availability section title', {
          source: 'jats-convert:parts',
        });
